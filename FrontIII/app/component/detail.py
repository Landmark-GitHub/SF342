...
def author_modal(df):
    ...
    .pub-eid    { color: #64748b; font-size: 12px; margin-top: 6px; }
    ...
    for _, row in df_author.iterrows():
        for col in ["evidence_titles", "evidence_paper_ids"]:
            val = row.get(col, [])
            if isinstance(val, str):
                try: val = ast.literal_eval(val)
                except: val = [val]
        titles = row.get("evidence_titles", [])
        eids   = row.get("evidence_paper_ids", [])
        for col, ref in [("evidence_titles", titles), ("evidence_paper_ids", eids)]:
            val = row.get(col, [])
            if isinstance(val, str):
                try: val = ast.literal_eval(val)
                except: val = [val]
            if col == "evidence_titles": titles = val
            else: eids = val
        for t, e in zip(titles, eids):
            papers.append({"title": t, "eid": e})

    paper_df = pd.DataFrame(papers).drop_duplicates().reset_index(drop=True)
    ...
    if paper_df.empty:
        ...
        for i, row in enumerate(paper_df.itertuples(), 1):
            ...
            <div class="pub-item">
                ...
                <div class="pub-eid">🔗 EID: {row.eid}</div>
            </div>
            """
